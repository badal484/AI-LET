...
export class ElevenLabsTTSProvider implements ITextToSpeechProvider {
  ...
  public async synthesize(text: string, options: TTSOptions): Promise<TTSSynthesisResult> {
    ...
    if (!this.apiKey) {
      logger.warn('[ElevenLabsTTSProvider] No API key configured, falling back to simulated speech synthesis');
      ...
      const costUsd = (chars / 1000) * SYSTEM_CONSTANTS.VOICE.PRICING.TTS_PER_1K_CHARS_USD.ELEVENLABS_TURBO;
    ...
    try {
      ...
      logger.error(`[ElevenLabsTTSProvider] Error: ${err.message}`);
      throw err;
    }
  }
  ...
  public async streamSynthesis(
    ...
    if (!this.apiKey) {
      ...
      return {
        ...
        charactersCount: chars,
        latencyMs: Date.now() - startTime,
        costUsd: (chars / 1000) * SYSTEM_CONSTANTS.VOICE.PRICING.TTS_PER_1K_CHARS_USD.ELEVENLABS_TURBO,
      };
    }
